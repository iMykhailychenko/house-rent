...
const Tooltip = ({ children, withMobile = true, className, content, hidden = false }: IProps): JSX.Element => {
    ...
    const handleTooltipPosition = useCallback((): void => {
        ...
        const isLeft = ref.current?.getBoundingClientRect().left / window.innerHeight > 0.4;
    ...
    return (
        <div
            ref={ref}
            className={clsx(css.wrp, className, { [css.mobile]: withMobile })}
            onFocus={handleHover}
